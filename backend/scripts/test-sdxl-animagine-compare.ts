/** Animagine SDXL — 与 compare 测试同 prompt / 同 seed */
import fs from 'fs'
...
const OUT = path.resolve(__dirname, '../../data/static/images/compare_sdxl_animagine.png')
...
async function main() {
  ...
  const checkpoint = LOCAL_COMIC_ENV.sdxlPortraitCheckpoint
  ...
  console.log('[animagine] checkpoint:', checkpoint)
  console.log('[animagine] positive:', params.positive.slice(0, 220), '...')
  const prompt = loadWorkflowTemplate('sdxl_anime')
  injectSdxlWorkflow(prompt, params)
  const promptId = await queueComfyPrompt(prompt)
  console.log('[animagine] queued:', promptId)
  ...
  console.log('[animagine] OK', OUT, `${meta.width}x${meta.height}`)
  console.log('View: http://localhost:5679/static/images/compare_sdxl_animagine.png')
}
