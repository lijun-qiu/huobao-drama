...
/** LLM 写配图 prompt 时使用的全文连贯规则说明 */
export const NARRATION_PLOT_CONTINUITY_LLM_RULE =
  '【剧情】只写当前配图段的主画面动作与互动，禁止写入「前文概要」或罗列已过场情节；货物/道具/品类仅在其首次出现的旁白句之后才可在画面中出现，且仅保留与当前画面仍相关的物件（如夜市摆摊段可写摊位上的蛤蟆镜，已过场的推自行车情节勿写入）'

/** LLM 写片头标题图 prompt 时的规则 */
export const NARRATION_TITLE_IMAGE_LLM_RULE =
  '片头标题图：综合 full_narration 全文（片头 hook + 正文旁白），提炼最能概括本期人生剧本核心的【片头背景场景】与【主题氛围】；不要只写 hook 字面，要体现全文主线（经营品类、身份转折、核心事件、时代背景感等），预留中央叠字区域，绝对无文字'
...
export function parseNarrationIntoScenePlot(
  ...
  const plot = buildFocusedScenePlot(lines, priorLines, scene)
  return { scene, plot }
}

/** 按配图段旁白句组装完整场景 prompt（【场景】【剧情】智能推断 + 素体万能模板） */
...
export function buildMinimalPortraitPostureHint(variantLabel?: string | null): string {
  ...
  if (/童年|幼年|孩童|儿时|幼/.test(label)) {
    return `小型素体小人，${eyes}，站立，简单活泼姿态`
  }
  if (/青年|少年|年轻/.test(label)) {
    return `标准素体小人，${eyes}，站立或行走，可推手推车或自行车轮廓作道具`
  }
  if (/中年/.test(label)) {
    return `略宽素体小人，${eyes}，坐于柜台后，放松姿态，手持茶杯`
  }
  if (/老年|晚年|垂暮|苍老|年迈/.test(label)) {
    return `略佝偻素体小人，${eyes}，坐于凳上，手持圆扇`
  }
